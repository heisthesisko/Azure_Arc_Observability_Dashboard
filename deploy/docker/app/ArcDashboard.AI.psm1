...
$script:FoundryAzureConfigDirectory = Join-Path $dataRoot '.azure-foundry'
...
function Get-FoundryAccessToken {
    ...
    if ($DelegatedAccessToken) {
        if ($DelegatedAccessToken.Length -gt 16384 -or $DelegatedAccessToken -notmatch '^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$') {
            throw [InvalidOperationException]::new('The authentication proxy supplied an invalid delegated access token.')
        }
...
function Get-FoundryAuthenticationStatus {
    ...
    try {
        $null = Get-FoundryAccessToken -TenantId $TenantId -DelegatedAccessToken $DelegatedAccessToken
...
function Invoke-FoundryChat {
    ...
    $accessToken = Get-FoundryAccessToken -TenantId $Config.Ai.TenantId `
        -DelegatedAccessToken ([string]$SharedState.FoundryAccessToken)
    $systemMessage = @"
