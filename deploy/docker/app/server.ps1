param(
    ...
    [ValidateSet('LocalSingleUser', 'TrustedProxy')]
    [string]$AccessMode = 'LocalSingleUser',

    [ValidateRange(1, 50)]
    [int]$MaxUserSessions = 20,

    [string]$AdministratorUsers = $env:ARC_DASHBOARD_ADMIN_USERS,
...
# still protected at rest with AES-256-GCM and a Linux user-private local key.
$securityModulePath = Join-Path $PSScriptRoot 'ArcDashboard.Security.psm1'
Import-Module $securityModulePath -Force
$configPath = Join-Path $DataRoot 'dashboard.config.dat'
$legacyConfigPath = Join-Path $DataRoot 'dashboard.config.json'
$linuxSecretDirectory = Join-Path $DataRoot '.secrets'
$linuxMasterKeyPath = Join-Path $linuxSecretDirectory 'master.key'

function Protect-Configuration {
    param([Parameter(Mandatory)][string]$PlainText)

    return Protect-LinuxDashboardPayload -PlainText $PlainText `
        -KeyPath $linuxMasterKeyPath -Purpose 'configuration'
}
...
function Save-ProtectedConfiguration {
    ...
    $temporaryPath = Join-Path (Split-Path -Parent $configPath) ([IO.Path]::GetRandomFileName())
    try {
    ...
    Set-LinuxPrivateFilePermissions -Path $configPath
...
if (Test-Path -LiteralPath $configPath) {
    try {
        ...
        Write-Warning 'The saved dashboard configuration cannot be decrypted with this Linux deployment key. Setup is required again.'
...
$script:SharedState = [hashtable]::Synchronized(@{
    ...
    MonitoringStorePath  = Join-Path (Join-Path $DataRoot '.monitoring') 'session.dat'
    ...
    WorkloadStorePath    = Join-Path (Join-Path $DataRoot '.monitoring') 'workload.dat'
    WorkloadCollectionRequested = $false
    WorkloadLastError    = $null
    WorkloadGeneration   = [long]0
    UserStates           = [hashtable]::Synchronized(@{})
    RefreshSignal        = [hashtable]::Synchronized(@{ Requested = $false })
    LifecycleGate        = [Threading.SemaphoreSlim]::new(1, 1)
    Disabled             = $false
...
$workerDirectory = Join-Path $DataRoot '.runtime'
$foundryAzureConfigDirectory = Join-Path $DataRoot '.azure-foundry'
if (-not (Test-Path -LiteralPath $workerDirectory)) {
    New-Item -ItemType Directory -Path $workerDirectory -Force | Out-Null
}
Set-LinuxPrivateDirectoryPermissions -Path $workerDirectory
...
function Initialize-WorkerLogFiles {
    param([Parameter(Mandatory)][string]$Prefix)

    $paths = @(@('out', 'err') | ForEach-Object {
        Join-Path $workerDirectory "$Prefix.$_.log"
    })
...
function Start-AzureCliInstall {
    ...
    $guidance = & (Join-Path $PSScriptRoot 'Install-AzureCli.ps1') 2>&1
    return @{
        state = 'manual'
        message = ($guidance -join [Environment]::NewLine).Trim()
    }
}
...
function Start-AzureDeviceLogin {
    ...
    $logPaths = Initialize-WorkerLogFiles -Prefix 'login'
    $outputPath = $logPaths[0]
    $errorPath = $logPaths[1]
    $arguments = @(
...
function Stop-LoginProcessTree {
    param(
    ...
    try {
        ...
        Write-Warning "Could not terminate the $Name process tree: $($_.Exception.Message)"
    }
    catch [AggregateException] {
        Write-Warning "Could not terminate every process in the $Name process tree: $($_.Exception.Message)"
    }
    if (-not $Process.WaitForExit(5000)) {
        Write-Warning "The $Name process tree did not exit within five seconds."
    }
}
...
function Start-FoundryDeviceLogin {
        ...
        if ($AccessMode -eq 'TrustedProxy') {
            $authentication = Get-FoundryAuthenticationStatus -TenantId $validatedTenantId `
                -DelegatedAccessToken ([string]$UserState.FoundryAccessToken)
            if (-not $authentication.Authenticated) {
                throw 'The signed-in web session does not contain a delegated Microsoft Foundry token.'
            }
            return @{
                state = 'completed'
                authenticated = $true
        ...
        $logPaths = Initialize-WorkerLogFiles -Prefix 'foundry-login'
        $outputPath = $logPaths[0]
        $errorPath = $logPaths[1]
...
function Get-FoundryLoginStatus {
        ...
        if ($AccessMode -eq 'TrustedProxy') {
            ...
            return @{
                state = if ($authentication.Authenticated) { 'completed' } else { 'idle' }
...
function Clear-PerUserState {
    [Threading.Monitor]::Enter($script:SharedState.UserStates.SyncRoot)
    try {
        $states = @($script:SharedState.UserStates.Values)
    }
    finally {
        [Threading.Monitor]::Exit($script:SharedState.UserStates.SyncRoot)
    }
    foreach ($state in $states) {
        $lifecycleAcquired = $false
        $monitoringAcquired = $false
        $workloadAcquired = $false
        try {
            $lifecycleAcquired = $state.LifecycleGate.Wait([TimeSpan]::FromMinutes(2))
            if (-not $lifecycleAcquired) {
                throw 'Timed out waiting to stop per-user requests during scope reconfiguration.'
            }
            $state.Disabled = $true
            $monitoringAcquired = $state.MonitoringGate.Wait([TimeSpan]::FromMinutes(2))
            $workloadAcquired = $state.WorkloadGate.Wait([TimeSpan]::FromMinutes(2))
            if (-not $monitoringAcquired -or -not $workloadAcquired) {
                throw 'Timed out waiting to clear per-user monitoring state.'
            }
            Remove-MonitoringSnapshot -SharedState $state
            Remove-WorkloadMonitoringSnapshot -SharedState $state
            $state.FoundryAccessToken = ''
            Remove-Item -LiteralPath ([string]$state.UserRoot) -Recurse -Force -ErrorAction SilentlyContinue
        }
        finally {
            if ($workloadAcquired) { [void]$state.WorkloadGate.Release() }
            if ($monitoringAcquired) { [void]$state.MonitoringGate.Release() }
            if ($lifecycleAcquired) { [void]$state.LifecycleGate.Release() }
        }
    }
    $script:SharedState.UserStates.Clear()
}
...
function Save-AiConfiguration {
    ...
    $validatedTenantId = Get-ValidatedFoundryTenantId -TenantId $TenantId
    $authentication = Get-FoundryAuthenticationStatus -TenantId $validatedTenantId `
        -DelegatedAccessToken $(if ($UserState) { [string]$UserState.FoundryAccessToken } else { '' })
    if (-not $authentication.Authenticated) {
...
function Invoke-SecureLogout {
    param([hashtable]$UserState)

    if ($AccessMode -eq 'TrustedProxy') {
        if ($UserState -and $UserState.UserKey) {
            $lifecycleAcquired = $false
            $monitoringAcquired = $false
            $workloadAcquired = $false
            try {
                $lifecycleAcquired = $UserState.LifecycleGate.Wait([TimeSpan]::FromMinutes(10))
                if (-not $lifecycleAcquired) {
                    throw 'Timed out waiting for active dashboard requests to finish during logout.'
                }
                $UserState.Disabled = $true
                $monitoringAcquired = $UserState.MonitoringGate.Wait([TimeSpan]::FromMinutes(2))
                $workloadAcquired = $UserState.WorkloadGate.Wait([TimeSpan]::FromMinutes(2))
                if (-not $monitoringAcquired -or -not $workloadAcquired) {
                    throw 'Timed out waiting for the current monitoring collection to finish during logout.'
                }
                Remove-MonitoringSnapshot -SharedState $UserState
                Remove-WorkloadMonitoringSnapshot -SharedState $UserState
                $UserState.FoundryAccessToken = ''
                $script:SharedState.UserStates.Remove([string]$UserState.UserKey)
                Remove-Item -LiteralPath ([string]$UserState.UserRoot) -Recurse -Force -ErrorAction SilentlyContinue
            }
            finally {
                if ($workloadAcquired) { [void]$UserState.WorkloadGate.Release() }
                if ($monitoringAcquired) { [void]$UserState.MonitoringGate.Release() }
                if ($lifecycleAcquired) { [void]$UserState.LifecycleGate.Release() }
            }
        }
        return @{
            loggedOut = $true
            serverStopping = $false
            proxySignOutUrl = '/oauth2/sign_out?rd=/signed-out.html'
        }
    }

    $localLifecycleAcquired = $script:SharedState.LifecycleGate.Wait([TimeSpan]::FromMinutes(10))
    if (-not $localLifecycleAcquired) {
        throw 'Timed out waiting for active dashboard requests to finish during logout.'
    }
    try {
    $script:SharedState.Disabled = $true
    ...
    Stop-LoginProcessTree -Process $script:LoginProcess -Name 'Arc sign-in'
    Stop-LoginProcessTree -Process $script:FoundryLoginProcess -Name 'Foundry sign-in'
    ...
    Remove-MonitoringSnapshot -SharedState $script:SharedState
    Remove-WorkloadMonitoringSnapshot -SharedState $script:SharedState
    Remove-Item -LiteralPath $linuxSecretDirectory -Recurse -Force -ErrorAction SilentlyContinue
    ...
    return @{ loggedOut = $true; serverStopping = $true }
        ...
        [void]$script:SharedState.LifecycleGate.Release()
    }
}
...
param($SharedState, $UserState, $Method, $Path, $QueryValues, $Body, $Client)
$lifecycleAcquired = $false
try {
    $stream = $Client.GetStream()
    $usesUserState = $Path -eq '/api/monitoring' -or $Path.StartsWith('/api/monitoring/') -or
        $Path -eq '/api/workload-monitoring' -or $Path.StartsWith('/api/workload-monitoring/') -or
        $Path -eq '/api/ai/chat'
    if ($usesUserState) {
        $lifecycleAcquired = $UserState.LifecycleGate.Wait([TimeSpan]::FromMinutes(2))
        if (-not $lifecycleAcquired) {
            Write-JsonResponseBytes -Stream $stream -Value @{ error = 'Timed out waiting for the user session.' } -StatusCode 503 -StatusText 'Service Unavailable'
            return
        }
        if ($UserState.Disabled) {
            Write-JsonResponseBytes -Stream $stream -Value @{ error = 'The user session has ended.' } -StatusCode 409 -StatusText 'Conflict'
            return
        }
    }
    if ($Path -eq '/api/monitoring' -or $Path.StartsWith('/api/monitoring/')) {
        Invoke-MonitoringApiRequest -SharedState $UserState -Method $Method -Path $Path -Body $Body -Stream $stream
    }
    elseif ($Path -eq '/api/workload-monitoring' -or $Path.StartsWith('/api/workload-monitoring/')) {
        Invoke-WorkloadMonitoringApiRequest -SharedState $UserState -Method $Method -Path $Path -Body $Body -Stream $stream
    }
    elseif ($Path -eq '/api/ai/chat') {
        Invoke-StandaloneAiApiRequest -SharedState $UserState -Method $Method -Path $Path -Body $Body -Stream $stream
    }
    else {
    ...
    if ($lifecycleAcquired) {
        [void]$UserState.LifecycleGate.Release()
    }
    $Client.Dispose()
}
'@
...
function Invoke-EnterpriseApiRequestAsync {
    param($Method, $Path, $QueryValues, $Body, $Client, $UserState)
...
function Get-RequestUserState {
    param([hashtable]$Headers)

    if ($AccessMode -ne 'TrustedProxy') {
        return $script:SharedState
    }

    $identity = if ($Headers.ContainsKey('x-auth-request-user')) {
        [string]$Headers['x-auth-request-user']
    }
    else {
        [string]$Headers['x-forwarded-user']
    }
    if ([string]::IsNullOrWhiteSpace($identity) -or $identity.Length -gt 256) {
        throw [UnauthorizedAccessException]::new('The authentication proxy did not provide a valid user identity.')
    }
    $normalizedIdentity = $identity.Trim().ToLowerInvariant()
    $identityBytes = [Text.Encoding]::UTF8.GetBytes($normalizedIdentity)
    try {
        $userKey = [Convert]::ToHexString([Security.Cryptography.SHA256]::HashData($identityBytes)).ToLowerInvariant()
    }
    finally {
        [Array]::Clear($identityBytes, 0, $identityBytes.Length)
    }

    if (-not $script:SharedState.UserStates.ContainsKey($userKey)) {
        $inactiveBefore = [datetime]::UtcNow.AddHours(-8)
        foreach ($existingKey in @($script:SharedState.UserStates.Keys)) {
            $existingState = $script:SharedState.UserStates[$existingKey]
            $monitoringActive = $existingState.MonitoringSnapshot -and
                [string]$existingState.MonitoringSnapshot.state -eq 'active'
            $workloadActive = $existingState.WorkloadSnapshot -and
                [string]$existingState.WorkloadSnapshot.state -eq 'active'
            if (-not $monitoringActive -and -not $workloadActive -and
                [datetime]$existingState.LastAccessedAt -lt $inactiveBefore) {
                $script:SharedState.UserStates.Remove($existingKey)
            }
        }
        if ($script:SharedState.UserStates.Count -ge $MaxUserSessions) {
            throw [InvalidOperationException]::new('The configured concurrent dashboard user limit has been reached.')
        }
        $userRoot = Join-Path (Join-Path $DataRoot 'users') $userKey
        $monitoringRoot = Join-Path $userRoot '.monitoring'
        New-Item -ItemType Directory -Path $monitoringRoot -Force | Out-Null
        Set-LinuxPrivateDirectoryPermissions -Path $userRoot
        Set-LinuxPrivateDirectoryPermissions -Path $monitoringRoot
        $userState = [hashtable]::Synchronized(@{
            Config = $script:SharedState.Config
            OperationsSnapshot = $script:SharedState.OperationsSnapshot
            SqlSnapshot = $script:SharedState.SqlSnapshot
            KubernetesSnapshot = $script:SharedState.KubernetesSnapshot
            BuildGate = $script:SharedState.BuildGate
            SqlBuildGate = $script:SharedState.SqlBuildGate
            KubernetesBuildGate = $script:SharedState.KubernetesBuildGate
            VmSkuGate = $script:SharedState.VmSkuGate
            VmSkuCache = $script:SharedState.VmSkuCache
            LastOperationsError = $script:SharedState.LastOperationsError
            LastSqlError = $script:SharedState.LastSqlError
            LastKubernetesError = $script:SharedState.LastKubernetesError
            MonitoringSnapshot = $null
            MonitoringGate = [Threading.SemaphoreSlim]::new(1, 1)
            MonitoringStateGate = [Threading.SemaphoreSlim]::new(1, 1)
            MonitoringStorePath = Join-Path $monitoringRoot 'session.dat'
            MonitoringCollectionRequested = $false
            MonitoringLastError = $null
            MonitoringGeneration = [long]0
            WorkloadSnapshot = $null
            WorkloadGate = [Threading.SemaphoreSlim]::new(1, 1)
            WorkloadStateGate = [Threading.SemaphoreSlim]::new(1, 1)
            WorkloadStorePath = Join-Path $monitoringRoot 'workload.dat'
            WorkloadCollectionRequested = $false
            WorkloadLastError = $null
            WorkloadGeneration = [long]0
            FoundryConfigDirectory = Join-Path $userRoot '.azure-foundry'
            FoundryAccessToken = ''
            UserKey = $userKey
            UserRoot = $userRoot
            LastAccessedAt = [datetime]::UtcNow
            Disabled = $false
            RefreshSignal = $script:SharedState.RefreshSignal
            LifecycleGate = [Threading.SemaphoreSlim]::new(1, 1)
            ConfigVersion = $script:SharedState.ConfigVersion
            ForceRefresh = $false
            ShutdownRequested = $false
        })
        $userState.MonitoringSnapshot = Restore-MonitoringSnapshot -Path $userState.MonitoringStorePath
        if ($userState.MonitoringSnapshot) {
            $userState.MonitoringGeneration = [long]$userState.MonitoringSnapshot.generation
        }
        $userState.WorkloadSnapshot = Restore-WorkloadMonitoringSnapshot -Path $userState.WorkloadStorePath
        if ($userState.WorkloadSnapshot) {
            $userState.WorkloadGeneration = [long]$userState.WorkloadSnapshot.generation
        }
        $script:SharedState.UserStates[$userKey] = $userState
    }

    $state = $script:SharedState.UserStates[$userKey]
    $state.Config = $script:SharedState.Config
    $state.ConfigVersion = $script:SharedState.ConfigVersion
    $state.OperationsSnapshot = $script:SharedState.OperationsSnapshot
    $state.SqlSnapshot = $script:SharedState.SqlSnapshot
    $state.KubernetesSnapshot = $script:SharedState.KubernetesSnapshot
    $state.LastOperationsError = $script:SharedState.LastOperationsError
    $state.LastSqlError = $script:SharedState.LastSqlError
    $state.LastKubernetesError = $script:SharedState.LastKubernetesError
    $state.FoundryAccessToken = if ($Headers.ContainsKey('x-auth-request-access-token')) {
        [string]$Headers['x-auth-request-access-token']
    }
    elseif ($Headers.ContainsKey('x-forwarded-access-token')) {
        [string]$Headers['x-forwarded-access-token']
    }
    else {
        ''
    }
...
$sessionCookieName = 'ArcDashboardSession'
$sessionTokenBytes = [byte[]]::new(32)
[Security.Cryptography.RandomNumberGenerator]::Fill($sessionTokenBytes)
try {
    $sessionToken = [Convert]::ToBase64String($sessionTokenBytes).TrimEnd('=').Replace('+', '-').Replace('/', '_')
}
finally {
...
function Test-SessionCookie {
    ...
    $escapedName = [regex]::Escape($sessionCookieName)
    $match = [regex]::Match([string]$Headers['cookie'], "(?:^|;\s*)$escapedName=([^;]+)")
    if (-not $match.Success) {
        return $false
    }
    $actual = [Text.Encoding]::UTF8.GetBytes($match.Groups[1].Value)
    $expected = [Text.Encoding]::UTF8.GetBytes($sessionToken)
    try {
        return $actual.Length -eq $expected.Length -and
...
function Test-RequestAuthorized {
    ...
    return Test-SessionCookie -Headers $Headers
}

function Test-AdministratorRequest {
    param([hashtable]$Headers)

    if ($AccessMode -eq 'LocalSingleUser') {
        return $true
    }
    $administrators = @($AdministratorUsers -split ',' | ForEach-Object {
        $_.Trim().ToLowerInvariant()
    } | Where-Object { $_ })
    if ($administrators.Count -eq 0) {
        return $false
    }
    $identity = if ($Headers.ContainsKey('x-auth-request-user')) {
        [string]$Headers['x-auth-request-user']
    ...
    return $identity.Trim().ToLowerInvariant() -in $administrators
}
...
function Write-HttpResponse {
    ...
    $headers = "HTTP/1.1 $StatusCode $StatusText`r`nContent-Type: $ContentType`r`nContent-Length: $($Body.Length)`r`n${cookieHeader}Cache-Control: no-store`r`nPragma: no-cache`r`nX-Content-Type-Options: nosniff`r`nReferrer-Policy: no-referrer`r`nConnection: close`r`n`r`n"
...
function Test-AllowedHostHeader {
    ...
    $hostHeader = ([string]$Headers['host']).Trim()
    if ($AccessMode -eq 'TrustedProxy') {
        [Uri]$parsedHost = $null
        return (
            $hostHeader.Length -le 255 -and
            [Uri]::TryCreate("http://$hostHeader/", [UriKind]::Absolute, [ref]$parsedHost) -and
            [string]::IsNullOrEmpty($parsedHost.UserInfo) -and
            $parsedHost.HostNameType -ne [UriHostNameType]::Unknown -and
            $parsedHost.AbsolutePath -eq '/' -and
            [string]::IsNullOrEmpty($parsedHost.Query) -and
            [string]::IsNullOrEmpty($parsedHost.Fragment)
        )
    }
    return $hostHeader.ToLowerInvariant() -in @(
        "localhost:$Port",
        "127.0.0.1:$Port"
    )
}

$listenAddress = [Net.IPAddress]::Parse($BindAddress)
$listener = [Net.Sockets.TcpListener]::new($listenAddress, $Port)
$listener.Start()
$displayHost = if ($BindAddress -eq '0.0.0.0') { 'localhost' } else { $BindAddress }
$url = "http://${displayHost}:$Port/"

Write-Host "Azure Arc Observability Dashboard running at $url (listening on ${BindAddress}:$Port)"
...
if (-not $NoBrowser) {
    $browserLauncher = Get-Command xdg-open -ErrorAction SilentlyContinue
    if ($browserLauncher) {
        Start-Process -FilePath $browserLauncher.Source -ArgumentList @($url)
    }
    else {
        Write-Warning "xdg-open is unavailable. Open $url in a browser on this Linux host."
    }
}
...
try {
    while (-not $script:SharedState.ShutdownRequested) {
        ...
        $requestDeadline = [datetime]::UtcNow.AddSeconds(15)
        ...
        try {
            ...
            while ($matchedTerminatorBytes -lt $headerTerminator.Length) {
                if ([datetime]::UtcNow -ge $requestDeadline) {
                    throw [TimeoutException]::new('The request header deadline was exceeded.')
                }
            ...
            if ($contentLength -gt 0) {
                ...
                while ($offset -lt $contentLength) {
                    if ([datetime]::UtcNow -ge $requestDeadline) {
                        throw [TimeoutException]::new('The request body deadline was exceeded.')
                    }
            ...
            if ($path -eq '/health/live' -and $method -eq 'GET') {
                Write-JsonResponse $stream @{ status = 'live' }
                continue
            }
            if ($path -eq '/health/ready' -and $method -eq 'GET') {
                $workersRunning = -not $script:MonitoringHandle.IsCompleted -and
                    -not $script:WorkloadHandle.IsCompleted -and
                    -not $script:RefreshHandle.IsCompleted
                $ready = [bool]$script:SharedState.Config.IsConfigured -and $workersRunning
                $statusCode = if ($ready) { 200 } else { 503 }
                $statusText = if ($ready) { 'OK' } else { 'Service Unavailable' }
                Write-JsonResponse $stream @{
                    status = if ($ready) { 'ready' } else { 'configuring' }
                } $statusCode $statusText
                continue
            }

            if ($path.StartsWith('/api/') -and -not (Test-RequestAuthorized -Headers $requestHeaders)) {
                Write-JsonResponse $stream @{ error = 'A valid dashboard browser session is required.' } 403 'Forbidden'
                continue
            }
            $requestUserState = if ($path.StartsWith('/api/')) {
                try {
                    Get-RequestUserState -Headers $requestHeaders
                }
                catch {
                    Write-JsonResponse $stream @{ error = $_.Exception.Message } 503 'Service Unavailable'
                    continue
                }
            }
            else {
            ...
            $administratorOnly = $path -in @(
                '/api/setup/install-cli',
                '/api/setup/login',
                '/api/setup/login-status',
                '/api/setup/resource-groups',
                '/api/setup/configure',
                '/api/ai/configure'
            )
            if ($administratorOnly -and -not (Test-AdministratorRequest -Headers $requestHeaders)) {
                Write-JsonResponse $stream @{ error = 'A dashboard administrator identity is required for this operation.' } 403 'Forbidden'
                continue
            }
            ...
            if ($method -ne 'GET' -and $path.StartsWith('/api/') -and $requestHeaders.ContainsKey('origin')) {
                ...
                if ($AccessMode -eq 'TrustedProxy' -and $requestHeaders.ContainsKey('x-forwarded-host')) {
                    $forwardedScheme = if ($requestHeaders.ContainsKey('x-forwarded-proto')) {
                        [string]$requestHeaders['x-forwarded-proto']
                    }
                    else {
                        'https'
                    }
                    $allowedOrigins += "${forwardedScheme}://$($requestHeaders['x-forwarded-host'])"
                }
            ...
            if (Test-EnterpriseApiPath -Path $path) {
                ...
                Invoke-EnterpriseApiRequestAsync -Method $method -Path $path -QueryValues $queryValues -Body $requestBody -Client $client -UserState $requestUserState
            ...
            elseif ($path -eq '/api/ai/login' -and $method -eq 'POST') {
                try {
                    ...
                    Write-JsonResponse $stream (Start-FoundryDeviceLogin -TenantId ([string]$request.tenantId) -UserState $requestUserState)
                }
                catch {
            ...
            elseif ($path -eq '/api/ai/login-status' -and $method -eq 'GET') {
                try {
                    Write-JsonResponse $stream (Get-FoundryLoginStatus -TenantId ([string]$queryValues.tenantId) -UserState $requestUserState)
                }
                catch {
                    Write-JsonResponse $stream @{ error = $_.Exception.Message } 400 'Bad Request'
                }
            ...
            elseif ($path -eq '/api/ai/configure' -and $method -eq 'POST') {
                try {
                    ...
                    Write-JsonResponse $stream (Save-AiConfiguration -TenantId ([string]$request.tenantId) `
                        -Endpoint ([string]$request.endpoint) -Model ([string]$request.model) -UserState $requestUserState)
                }
                catch {
                    Write-JsonResponse $stream @{ error = $_.Exception.Message } 400 'Bad Request'
                }
            ...
            elseif ($path -eq '/api/logout' -and $method -eq 'POST') {
                try {
                    Write-JsonResponse $stream (Invoke-SecureLogout -UserState $requestUserState)
                }
                catch {
                    Write-JsonResponse $stream @{ error = $_.Exception.Message } 500 'Internal Server Error'
                }
            ...
            elseif ($path -eq '/signed-out.html') {
                Write-HttpResponse $stream 200 'OK' 'text/html; charset=utf-8' ([IO.File]::ReadAllBytes((Join-Path $PSScriptRoot 'signed-out.html')))
            }
            else {
            ...
            Write-Verbose "Client request deadline exceeded: $($_.Exception.Message)"
        }
        finally {
