...
$script:MonitoringEntropy = [Text.Encoding]::UTF8.GetBytes('ArcDashboardMonitoring:v1')

function New-MonitoringAesGcm {
    ...
    try {
        ...
        # PowerShell 7.0-7.3 use the earlier .NET constructor; both use a 16-byte tag below.
        return [Security.Cryptography.AesGcm]::new($Key)
    }
}
...
function Protect-MonitoringPayload {
    ...
    try {
        $aes = New-MonitoringAesGcm -Key $key
        try {
        ...
        $wrappedKey = [Security.Cryptography.ProtectedData]::Protect(
            $key,
            $script:MonitoringEntropy,
            [Security.Cryptography.DataProtectionScope]::CurrentUser
        )
        return [ordered]@{
            version = 1
            algorithm = 'AES-256-GCM+DPAPI-CurrentUser'
...
function Unprotect-MonitoringPayload {
    ...
    if ([int]$envelope.version -ne 1 -or [string]$envelope.algorithm -ne 'AES-256-GCM+DPAPI-CurrentUser') {
        throw [IO.InvalidDataException]::new('The monitoring snapshot encryption envelope is not supported.')
    }
...
function Start-ServerMonitoringLoop {
    ...
    while (-not $SharedState.ShutdownRequested) {
        try {
            $session = $SharedState.MonitoringSnapshot
            if ($session -and [string]$session.state -eq 'active') {
                ...
                $due = $retryDue -and (
                    $SharedState.MonitoringCollectionRequested -or -not $session.lastCollectedAt -or
                    ([datetime]$session.lastCollectedAt).ToUniversalTime().AddSeconds($IntervalSeconds) -le $now
                )
                if ($due) {
                    Update-ServerMonitoringSession -SharedState $SharedState
                }
            }
        }
        catch {
