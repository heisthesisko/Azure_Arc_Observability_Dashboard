...
function Invoke-AzJson {
    ...
    $az = Get-AzExecutable
    if (-not $az) {
        throw 'Azure CLI is not installed. Open the dashboard setup page to install it.'
    }
...
function Set-AzSubscriptionContext {
    ...
    $mutex = [Threading.Mutex]::new($false, 'Local\EnterpriseArcDashboard-AzureCliContext')
    try {
...
function Start-DashboardRefreshLoop {
    ...
    while (-not $SharedState.ShutdownRequested) {
        ...
        $waited = 0
        while ($waited -lt $IntervalSeconds -and -not $SharedState.ShutdownRequested -and -not $SharedState.ForceRefresh) {
