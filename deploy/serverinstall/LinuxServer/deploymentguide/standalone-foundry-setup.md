# Linux Standalone Microsoft Foundry Setup and Usage Guide
...
Linux host running the dashboard.
...
| Foundry tenant | `.azure-foundry/` in the Linux deployment directory | Foundry model token and inference |
...
From the Linux deployment directory:
...
AES-256-GCM-protected configuration:
...
- Encrypts the separate single-server monitoring snapshot with AES-256-GCM
  using the Linux deployment's mode-`0600` local key
