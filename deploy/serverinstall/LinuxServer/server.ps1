...
$linuxSecretDirectory = Join-Path $PSScriptRoot '.secrets'
...
$script:SharedState = [hashtable]::Synchronized(@{
    ...
    MonitoringStorePath  = Join-Path (Join-Path $PSScriptRoot '.monitoring') 'session.dat'
    ...
    WorkloadStorePath    = Join-Path (Join-Path $PSScriptRoot '.monitoring') 'workload.dat'
...
$workerDirectory = [IO.Directory]::CreateTempSubdirectory('ArcDashboard-').FullName
...
try {
    while (-not $script:SharedState.ShutdownRequested) {
        ...
        try {
            ...
            if ($path.StartsWith('/api/') -and -not (Test-SessionCookie -Headers $requestHeaders)) {
